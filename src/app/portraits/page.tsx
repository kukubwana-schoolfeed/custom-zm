    ...
    <>
      ...
      <section className="relative min-h-[85vh] flex items-end pb-20 overflow-hidden bg-[#0A0A0A]">
        ...
        <div className="absolute inset-0 z-[0]">
          <Image
            src="/assets/portraits/portrait-print-lusaka-zambia-15.webp"
        ...
        <div className="relative z-[3] max-w-7xl mx-auto px-6 pt-24 w-full">
          ...
          <h1 className="headline headline-chrome mb-6" style={{ fontSize: 'clamp(3rem, 10vw, 8rem)' }}>
            YOUR STORY.<br /><span className="text-[var(--steel-bright)]">PERMANENTLY PRINTED.</span>
          </h1>
          ...
          <div className="columns-2 sm:columns-3 lg:columns-4 gap-3 space-y-3">
            {GALLERY_CURATED.map((src, i) => (
              <ScrollReveal key={i} delay={(i % 4) * 0.05}>
                <WrapReveal className="relative neon-border group break-inside-avoid mb-3">
            ...
            {USE_CASES.map((item, i) => (
              <ScrollReveal key={item.title} delay={i * 0.1}>
                <div className="neon-border bg-[#141414] p-8 text-center">
                  ...
                  <h3 className="headline headline-chrome text-xl mb-3">{item.title}</h3>
      ...
      <section className="py-24 px-6 bg-[#111111]">
        <div className="max-w-3xl mx-auto">
          <ScrollReveal>
            ...
            <PlotterLine center />
            <h2 className="headline headline-chrome text-center mb-4" style={{ fontSize: 'clamp(2.5rem, 6vw, 5rem)' }}>
              CHOOSE YOUR SIZE
            </h2>
          ...
          <ScrollReveal delay={0.1}>
            <div className="border border-[rgba(157,184,207,0.2)] bg-[#111111]">
              <div className="px-6 py-5 border-b border-[rgba(157,184,207,0.1)]">
                <span className="font-display headline headline-chrome text-xl tracking-wider">PORTRAIT PRINT SIZES & PRICING</span>
              </div>
              <div className="px-6 pb-6 pt-4 space-y-3">
                <div className="grid grid-cols-3 gap-4 pb-2 border-b border-[rgba(157,184,207,0.1)]">
                ...
                {PRICING.portraits.map((item, i) => (
                  <div key={i} className="grid grid-cols-3 gap-4 py-1 border-b border-[rgba(255,255,255,0.03)] last:border-b-0">
                    ...
                    <span className="text-[var(--steel-bright)] font-semibold">K{item.price}</span>
                    <a href={WA_LINKS.portrait} target="_blank" rel="noopener noreferrer"
                      className="text-[var(--steel)] text-xs hover:text-[var(--steel-bright)] font-semibold transition-colors">
                      Order &rarr;
                    </a>
                  </div>
                ))}
              </div>
              <div className="px-6 py-4 bg-[#0F0F0F] border-t border-[rgba(157,184,207,0.1)]">
      ...
      <section className="py-24 px-6 bg-[#0A0A0A] text-center">
        <div className="max-w-3xl mx-auto">
          <ScrollReveal>
            <h2 className="headline headline-chrome mb-6" style={{ fontSize: 'clamp(2.5rem, 7vw, 6rem)' }}>
              MAKE IT<br /><span className="text-[var(--steel-bright)]">PERMANENT.</span>
            </h2>
